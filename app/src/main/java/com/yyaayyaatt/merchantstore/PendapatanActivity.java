...
public class PendapatanActivity extends AppCompatActivity {
    ...
    TextView tv_lap_penjualan_total, tv_lap_penjualan_untung;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mContext = PendapatanActivity.this;
        mRecycler = findViewById(R.id.rv_rekap_pendapatan);
        tv_lap_penjualan_total = findViewById(R.id.tv_rekap_pendapatan_total);
        tv_lap_penjualan_untung = findViewById(R.id.tv_rekap_pendapatan_untung);
        btn_pick_1 = findViewById(R.id.btn_rekap_pendapatan_tgl_awal);
        btn_pick_2 = findViewById(R.id.btn_rekap_pendapatan_tgl_ahir);
        tv_date_awal = findViewById(R.id.tv_rekap_pendapatan_tgl_awal);
        tv_date_ahir = findViewById(R.id.tv_rekap_pendapatan_tgl_ahir);
        btn_cari = findViewById(R.id.btn_rekap_pendapatan_cari);
        ...
        tv_date_awal.setText(year1 + "-" + (String.format("%02d", month1 + 1)) + "-" + String.format("%02d", day1));
        tv_date_ahir.setText(year1 + "-" + (String.format("%02d", month1 + 1)) + "-" + String.format("%02d", day1));
...
//                            tv_date_ahir.setText(dayOfMonth + "-" + (monthOfYear + 1) + "-" + year);
    ...
    private void getPendapatan(String tgl_awal, String tgl_ahir) {
        transaksis.clear();
        Call<ResponseTransaksi> getdata = mApiService.getRekapPendapatan(tgl_awal, tgl_ahir);
                ...
                if (response.isSuccessful()) {
                    ...
                    if (response.body().getmKode().equals("1")) {
                        ...
                        if (!transaksis.isEmpty()) {
                            mAdapter = new PendapatanRecylerAdapter(mContext, transaksis);
                            mRecycler.setAdapter(mAdapter);
                            double total = 0, untung = 0;
                            for (int x = 0; x < transaksis.size(); x++) {
                                untung += Double.parseDouble(transaksis.get(x).getUntung());
                                tv_lap_penjualan_untung.setText("Laba Total:   Rp." + nf.format(untung));
                                total += Double.parseDouble(transaksis.get(x).getTotal()) - Double.parseDouble(transaksis.get(x).getDiskon());
