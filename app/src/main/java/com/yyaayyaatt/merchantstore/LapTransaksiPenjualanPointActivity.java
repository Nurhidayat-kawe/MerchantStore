...
import com.yyaayyaatt.merchantstore.adapter.RekapTransaksiRecylerAdapter;
...
public class LapTransaksiPenjualanPointActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_lap_transaksi_penjualan_point);
        ActionBar actionBar;
        actionBar = getSupportActionBar();
        ColorDrawable colorDrawable = new ColorDrawable(Color.parseColor("#F5005A7E"));
        actionBar.setBackgroundDrawable(colorDrawable);

        context = LapTransaksiPenjualanPointActivity.this;
        btn_pick_1 = findViewById(R.id.btn_rekap_penjualan_point_tgl_awal);
        btn_pick_2 = findViewById(R.id.btn_rekap_penjualan_point_tgl_ahir);
        tv_date_awal = findViewById(R.id.tv_rekap_penjualan_point_tgl_awal);
        tv_date_ahir = findViewById(R.id.tv_rekap_penjualan_point_tgl_ahir);
        btn_cari = findViewById(R.id.btn_rekap_penjualan_point_cari);
        mRecycler = findViewById(R.id.rv_rekap_penjualan_point_all);
        tv_lap_penjualan_total = findViewById(R.id.tv_rekap_pendapatan_point_total);
...
//                            tv_date_ahir.setText(dayOfMonth + "-" + (monthOfYear + 1) + "-" + year);
    ...
    private void getPendapatan(String tgl_awal, String tgl_ahir) {
        transaksis.clear();
        Call<ResponseTransaksi> getdata = mApiService.getRekapTransaksiPoint(tgl_awal, tgl_ahir);
                ...
                if (response.isSuccessful()) {
                    ...
                    if (response.body().getmKode().equals("1")) {
                        ...
                        if (!transaksis.isEmpty()) {
                            mAdapter = new RekapTransaksiRecylerAdapter(context, transaksis);
